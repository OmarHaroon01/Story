...
public class WallController implements Initializable {
    ...
    @FXML
    void myProfileLabelClicked(MouseEvent event) throws IOException {
    ...
    void myUploadsLabelClicked(MouseEvent event) throws IOException {
        ...
        Scene scene = new Scene(root);
        Stage primaryStage = (Stage)this.myUploadsLabel.getScene().getWindow();
        primaryStage.setScene(scene);
        primaryStage.setTitle("Story");
        primaryStage.sizeToScene();
        primaryStage.setResizable(false);
    ...
    public void setImageInOrder(){
        ...
        int cnt = 0;
        HBox hbox = null;
        //Serializer.imageList = Serializer.deserializeImages();
        int size = Serializer.imageList.size();
        while(cnt < Serializer.imageList.size()){
            if (cnt % 3 == 0){
                hbox = new HBox();
            ...
            Image image = new Image(Serializer.imageList.get(size - cnt - 1).getPhotoPath());
            ...
            imageView.setOnMouseClicked(new EventHandler() {
                ...
                public void handle(Event event) {
                    try {
                        ViewController.getImage(Serializer.imageList.get(size - finalCnt - 1));
